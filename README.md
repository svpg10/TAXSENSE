...
The app starts on `http://localhost:3000` (or the port printed in the terminal). The Express server handles API routes and serves the Vite-built React frontend.
...
|---|---|---|
| `GEMINI_API_KEY` | ✅ Yes | Google Gemini API key for AI chat and PDF extraction |
| `APP_URL` | Optional | Hosted URL of the app (used for self-referential links) |
...
|---|---|---|
| `POST` | `/api/chat` | Sends a message to the Gemini-powered tax copilot |
| `POST` | `/api/extract` | Uploads a Form 16 PDF and returns extracted tax fields |

Both endpoints are defined in `server.ts` and include automatic retry with exponential backoff across multiple Gemini model variants.
